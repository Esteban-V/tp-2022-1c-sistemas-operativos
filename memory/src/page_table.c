...
uint32_t pageTable_getFrame(uint32_t PID, uint32_t pt1_entry, uint32_t pt2_entry)
{
    ...
    frame = pt2_entry < list_size(pt2->entries) ? ((t_page_entry *)list_get(pt2->entries, pt2_entry))->frame : -1;
    pthread_mutex_unlock(&pageTablesMut);
...
void swapFile_register(t_swapFile *sf, uint32_t pid, uint32_t pageNumber, int index)
{
